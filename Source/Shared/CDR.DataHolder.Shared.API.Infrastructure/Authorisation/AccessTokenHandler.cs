...
namespace CDR.DataHolder.Shared.API.Infrastructure.Authorisation
{
    public class AccessTokenHandler : AuthorizationHandler<AccessTokenRequirement>
    {
        ...
        private async Task<bool> CheckAccessToken()
        {
            ...
            if (_httpContextAccessor.HttpContext == null || !_httpContextAccessor.HttpContext.Request.Headers.TryGetValue("Authorization", out authHeader))
            {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                return json?.GetValue("active")?.Value<bool>() ?? false;
            }
            else
            {
                return false;
