...
namespace CDR.DataHolder.IdentityServer.Services
{
    public class AuthorizeRequestUriService : IAuthorizeRequestUriService
    {
        ...
        public async Task<IEndpointResult> ProcessAsync(string requestUriKey, string clientId, NameValueCollection parameters)
        {
            var persistedGrant = await _persistedGrantStore.GetAsync(requestUriKey);
            ...
            if (!clientId.Equals(persistedGrant.ClientId, StringComparison.OrdinalIgnoreCase))
            {
            ...
            bool paramValidatePass = ValidateParameters(parameters, validatedAuthorizeRequest.RequestObjectValues, out string failedParameter);
            ...
            if (!response.IsError)
            {
                ...
                if (!string.IsNullOrWhiteSpace(authorizeClaims.CdrArrangementId))
                {
                    ...
                    await _customGrantService.RemoveGrantsForCdrArrangementId(authorizeClaims.CdrArrangementId, clientId);
        ...
        private static AuthorizeRequestUriResult GenerateParameterComparisonFailedErrorResult(string failedParameter, ValidatedAuthorizeRequest validatedAuthorizeRequest)
        {
        ...
        private static bool ValidateParameters(
            ...
            Dictionary<string, string> requestObjectValues,
            out string failedParameter)
        {
            ...
            foreach (var key in keys)
            {
                ...
                // e.g. value could be separated by space, so handle.
