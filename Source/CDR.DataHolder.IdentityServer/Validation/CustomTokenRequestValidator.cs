	...
	public class CustomTokenRequestValidator : ICustomTokenRequestValidator
	{
		...
		public async Task ValidateAsync(CustomTokenRequestValidationContext context)
		{
			...
			if (!isCdrRegistration && !isRefreshTokenGrant)
			{
				...
				{
					LogError(validatedTokenRequest, "Invalid redirect_uri");
					await SetFailedResult(TokenErrors.InvalidRequest, ValidationCheck.TokenRequestInvalidUri, requestContext);
				}
			}
			...
			if (!string.IsNullOrEmpty(requestedScope) && isRefreshTokenGrant)
            {
				...
				validatedTokenRequest.RequestedScopes = existingScopes;
			}
		}
		...
		private void LogError(ValidatedTokenRequest validatedTokenRequest, string message = null)
		{
